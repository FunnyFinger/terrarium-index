...
// Returns a full Supabase URL when available so images load on hosted site.
function getPlantImagePath(plant) {
    ...
    if (plant.imageUrl) {
        path = plant.imageUrl;
    } else if (plant.images && Array.isArray(plant.images) && plant.images.length > 0) {
        path = plant.images[0];
    } else {
        try {
    ...
    if (!path) {
        ...
        if (!slug) return null;
        path = `images/plants/${slug}/${slug}-1.jpg`;
    }
    if (path && typeof window !== 'undefined' && window.imageUtils && typeof window.imageUtils.resolvePlantImageUrl === 'function') {
        path = window.imageUtils.resolvePlantImageUrl(path, plant);
    }
    return path;
}
...
function getPlantThumbnailPath(plant) {
    ...
    const slug = getPlantFolderSlug(plant);
    if (!slug) return null;
    let path = `images/plants/${slug}/thumb.jpg`;
    if (path && typeof window !== 'undefined' && window.imageUtils && typeof window.imageUtils.resolvePlantImageUrl === 'function') {
        path = window.imageUtils.resolvePlantImageUrl(path, plant);
    }
    return path;
...
function showTooltip(event, text, imagePath, nodeData = null) {
    ...
    if (imagePath) {
        ...
        let attemptCount = 0;
        const tooltipPlant = nodeData && nodeData.plantData && nodeData.plantData.plant;
        const resolveForTooltip = (p) => (typeof window !== 'undefined' && window.imageUtils && typeof window.imageUtils.resolvePlantImageUrl === 'function' && tooltipPlant)
            ? window.imageUtils.resolvePlantImageUrl(p, tooltipPlant) : p;
        img.onerror = function() {
            ...
            if (pathMatch) {
                ...
                if (attemptCount <= 3) {
                    ...
                    const resolvedNext = resolveForTooltip(nextPath);
                    const nextUrl = resolvedNext + separator + 'refresh=' + Date.now();
                    if (!this.src.includes(resolvedNext) && !this.src.includes(nextPath)) {
                ...
                if (pathMatch2 && attemptCount === 1) {
                    ...
                    const resolvedStandard = resolveForTooltip(standardPath);
                    const standardUrl = resolvedStandard + separator + 'refresh=' + Date.now();
                    if (!this.src.includes(resolvedStandard) && !this.src.includes(standardPath)) {
