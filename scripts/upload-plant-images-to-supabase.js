#!/usr/bin/env node
/**
 * Upload all plant images from images/plants/<slug>/ to Supabase Storage
 * bucket vivarium-assets under plants/<slug>/.
 *
 * Usage: node scripts/upload-plant-images-to-supabase.js
 *
 * Reads SUPABASE_URL and SUPABASE_ANON_KEY from js/config.js or env.
 * Requires bucket "vivarium-assets" to exist and allow uploads (public or anon policy).
 *
 * After a successful run: in script.js remove or empty PLANT_SLUGS_WITH_BUCKET_IMAGES
 * so all plants use Supabase image URLs (not only the previous 3).
 */
'use strict';
...
const PLANTS_IMG_DIR = path.join(ROOT, 'images', 'plants');
...
async function run() {
  ...
  if (!fs.existsSync(PLANTS_IMG_DIR)) {
    console.error('Not found:', PLANTS_IMG_DIR);
    process.exit(1);
  }

  const dirs = fs.readdirSync(PLANTS_IMG_DIR, { withFileTypes: true })
  ...
  for (const slug of dirs) {
    const dirPath = path.join(PLANTS_IMG_DIR, slug);
    ...
    for (const file of files) {
      totalFiles++;
      const objectPath = 'plants/' + slug + '/' + file;
